...
$stmt = $pdo->prepare('SELECT * FROM assign where cid = :prof ');
    $stmt->execute(array(":prof" => $cid));
$assignments = $stmt->fetchAll(PDO::FETCH_ASSOC);

if ( isset($_POST['topic']) && isset($_POST['link']) && isset($_POST['deadline']) ) {
    ...
    <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
      <div class="position-sticky pt-5 ">
        <ul class="nav flex-column">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="assignment.php">
...
?>
        ...
        <form method="post">      
          ...
          <div class="mb-3">
            <label for="recipient-name" class="col-form-label">Deadline Date:</label>
            <input type="date" name="deadline" class="form-control" min="'.date("Y-m-d").'">
          </div>
          <button type="submit" class="btn btn-primary m-auto d-block">Create Assignment</button>
        </form>';
      ?>
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
